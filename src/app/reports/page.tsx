...
export default async function ReportsPage() {
  ...
  try {
    ...
    totalOverdue = await db.email.count({
      where: {
        dueDate: { lt: now },
        status: { notIn: [EmailStatus.CLOSED, EmailStatus.ARCHIVED] }
      }
    });
  } catch (error) {
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-description">Analyze triage metrics, check queue volumes, and export records for auditing.</p>
        </div>
      ...
      {dbError ? (
        <div className="card" style={{ textAlign: 'center', padding: '40px' }}>
          <p style={{ color: 'var(--status-needs-action)' }}>Could not load system reports. Please ensure your database is running and migrated.</p>
        </div>
      ) : totalEmails === 0 ? (
        <div className="card" style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-secondary)' }}>
          ...
          <p style={{ fontSize: '13px', marginTop: '6px' }}>Manually add a few email records to see metrics here.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
            <div className="card" style={{ borderLeft: '4px solid var(--accent)' }}>
              <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Total Tracked Emails</span>
              <h2 style={{ fontSize: '32px', fontWeight: 700, marginTop: '8px', color: '#ffffff' }}>{totalEmails}</h2>
              <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '4px' }}>Accumulated database logs</p>
            </div>
            
            <div className="card" style={{ borderLeft: '4px solid var(--status-needs-action)' }}>
              <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Overdue Actions</span>
              <h2 style={{ fontSize: '32px', fontWeight: 700, marginTop: '8px', color: 'var(--status-needs-action)' }}>{totalOverdue}</h2>
              <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '4px' }}>Items past target completion date</p>
            </div>

            <div className="card" style={{ borderLeft: '4px solid var(--status-closed)' }}>
              <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Resolution Rate</span>
              <h2 style={{ fontSize: '32px', fontWeight: 700, marginTop: '8px', color: 'var(--status-closed)' }}>
                {getPercent(statusCounts.CLOSED)}
              </h2>
              <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '4px' }}>Percentage of closed items</p>
            </div>
          </div>

          {/* Visual breakout distributions */}
                ...
                {Object.entries(statusCounts).map(([status, count]) => {
                  ...
                  return (
                    <div key={status} style={{ fontSize: '13px' }}>
                      ...
                      {/* Bar track */}
                      <div style={{ height: '8px', backgroundColor: 'var(--panel-bg)', borderRadius: '4px', overflow: 'hidden' }}>
                        <div style={{ width: percent, height: '100%', backgroundColor: colorVar, borderRadius: '4px' }} />
                      </div>
                    </div>
                  );
                })}
